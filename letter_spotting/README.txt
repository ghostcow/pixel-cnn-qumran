Instructions for running letter spotting:

1. Copy the document to 'comp/Segmentation-free_Dataset/Documents'

2. The main function is 'run_comp'. When running the code for the first time make sure all the parameters are set properly
...
3. After running the first function 'read_pages' - the file 'P.mat' will be saved to 'comp/Segmentation-free_Dataset'.
   When running it again it would load 'P.mat' and not read the pages again. If you want it to run again delete 'P.mat'.

4. After running the function 'pre_process_pages2' - the file 'PPP.mat' will be saved to 'comp/Segmentation-free_Dataset'.
...
5. After running 'build_windows_parfor' - all candidates will be computed and 'Segments.mat' will be saved to
